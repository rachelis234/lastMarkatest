...
import { SolveMatchDataSourse } from 'src/app/Models/SolveMatchDataSourse';
import { MatRadioChange } from '@angular/material';

@Component({
  selector: 'app-match-question',
  templateUrl: './match-question.component.html',
  styleUrls: ['./match-question.component.scss']
})

export class MatchQuestionComponent implements OnInit {
  lastQues: Question = null;
  currColor: string;
  colors:string[]= [
...
selectedRow:number
  ...
  ngOnInit() {
    ...
    this.question = this.userService.solveTest.questions.filter(q => q.type_id == 3);

    ans: Answer
    this.question.forEach(element => {

      this.answers.push(this.userService.solveTest.answers.find(a => a.question_id == element.question_id));
...
this.selectedRow=i;
  ...
  fillDataSourse() {
    ...
    for (i = 0; i < len; i++) {
      ...
      elem = new SolveMatchDataSourse();
      rand = Math.floor(Math.random() * this.question.length);
      elem.questions = this.question[rand];
      this.question.splice(rand, 1);
      rand = Math.floor(Math.random() * this.answers.length);
      elem.answers = this.answers[rand];
      this.answers.splice(rand, 1);
